...
type Overview = { users: { id: string; email: string; input_tokens: number; output_tokens: number; total_tokens: number; request_count: number }[] };
...
function DailyBars({ values }: { values: { day: string; request_count: number; input_tokens: number; output_tokens: number }[] }) {
  const maximum = Math.max(...values.map((item) => item.input_tokens + item.output_tokens), 1);
  return <div className="mt-6"><div className="flex h-52 items-end gap-1">{values.map((item) => <div key={item.day} title={`${item.day}: ${item.input_tokens + item.output_tokens} tokens`} className="group flex min-w-0 flex-1 items-end"><div className="w-full rounded-t bg-gradient-to-t from-violet-600 to-cyan-400 transition-opacity group-hover:opacity-70" style={{ height: `${Math.max((item.input_tokens + item.output_tokens) / maximum * 100, 2)}%` }} /></div>)}</div><div className="mt-2 flex justify-between text-xs text-slate-500"><span>{values[0]?.day || ""}</span><span>Peak {number.format(maximum)} tokens/day</span><span>{values[values.length - 1]?.day || ""}</span></div></div>;
}
