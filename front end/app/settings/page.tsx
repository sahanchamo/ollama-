...
const skillTemplates = [
  { name: "Professional email writer", instructions: "Write clear, professional emails. Ask for missing context when necessary. Provide a useful subject line and concise, polite wording." },
  { name: "Study tutor", instructions: "Teach step by step at the user's level. Explain concepts simply, give short examples, then ask a practice question." },
  { name: "Translator", instructions: "Translate accurately while preserving meaning, tone, names, and formatting. Ask for the target language if it is unclear." },
  { name: "Resume and cover-letter writer", instructions: "Create truthful, tailored resumes and cover letters. Highlight measurable achievements and do not invent experience." },
  { name: "Business proposal writer", instructions: "Create structured business proposals with an executive summary, objectives, scope, timeline, deliverables, assumptions, and next steps." },
  { name: "Code assistant", instructions: "Help with code explanation, debugging, refactoring, tests, and secure implementation. Explain important tradeoffs and provide minimal code changes." },
  { name: "Document summarizer", instructions: "Summarize supplied documents faithfully. Identify key points, decisions, actions, risks, and unanswered questions." },
  { name: "Social-media content writer", instructions: "Write platform-appropriate social content with a hook, concise body, call to action, and optional relevant hashtags." },
] as const;
...
export default function SettingsPage() {
  ...
  function useTemplate(template: typeof skillTemplates[number]) { setSkillName(template.name); setSkillInstructions(template.instructions); setNotice(`Template loaded: ${template.name}. Review it, then create the skill set.`); }
  ...
  {activeTab === "settings" && <>
  <section className="mt-6 rounded-2xl border border-white/10 bg-[#2a2a2a] p-5"><h2 className="font-semibold">Ready-made skill templates</h2><p className="mt-1 text-sm text-slate-400">Choose a template to load it into the skill-set form below, then create it.</p><div className="mt-4 flex flex-wrap gap-2">{skillTemplates.map((template) => <button key={template.name} type="button" onClick={() => useTemplate(template)} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-200 hover:bg-white/10">{template.name}</button>)}</div></section>
